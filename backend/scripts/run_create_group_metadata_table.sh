...
print_color "blue" "Exécution du script SQL pour créer/recréer la table system_group_metadata..."
...
if [ -z "$DB_PASSWORD" ]; then
  ...
  docker run --rm -v "$SCRIPT_DIR:/scripts" \
    postgres:13 psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f /scripts/create_group_metadata_table.sql
  RESULT=$?
else
  # Avec mot de passe
  docker run --rm -v "$SCRIPT_DIR:/scripts" \
...
if [ $RESULT -eq 0 ]; then
  ...
  print_color "green" "La table system_group_metadata a été supprimée et recréée."
else
