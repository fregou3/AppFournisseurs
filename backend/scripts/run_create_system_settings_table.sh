...
if ! command -v psql &> /dev/null; then
  ...
  if command -v docker &> /dev/null; then
    ...
    echo "docker run --rm -v \"$(pwd):/scripts\" -e PGPASSWORD=\"\$DB_PASSWORD\" postgres:13 psql -h \$DB_HOST -p \$DB_PORT -U \$DB_USER -d \$DB_NAME -f /scripts/create_system_settings_table.sql"
  fi
