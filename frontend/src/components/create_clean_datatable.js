/**
 * Script pour créer une version propre de DataTable.js avec pagination
 */
...
console.log(`Préparation d'une version propre de ${dataTablePath}...`);

// Créer une sauvegarde du fichier actuel
const backupPath = `${dataTablePath}.backup.${Date.now()}`;
if (fs.existsSync(dataTablePath)) {
  fs.writeFileSync(backupPath, fs.readFileSync(dataTablePath, 'utf8'));
  console.log(`Sauvegarde créée: ${backupPath}`);
}

// Contenu propre pour DataTable.js avec pagination
...
const DataTable = ({ 
  ...
  }, [groupName, filteredData, tableName, closeGroupDialog]);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Dialog open={openGroupDialog} onClose={closeGroupDialog}>
        ...
        <DialogActions>
          <Button onClick={closeGroupDialog}>Annuler</Button>
...
console.log(`Fichier ${dataTablePath} recréé avec succès!`);
