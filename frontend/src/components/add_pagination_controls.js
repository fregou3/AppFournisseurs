...
// Ajouter les contrôles de pagination après le composant DataTable
const dataTablePattern = /<\/DataTable>\s*\n\s*<\/Box>\s*\n\s*\);\s*\n\s*\};\s*\n\s*export default Home;/;
...
if (dataTablePattern.test(content)) {
  content = content.replace(dataTablePattern, paginationControls);
  console.log('Contrôles de pagination ajoutés avec succès');
} else {
  console.log('Pattern non trouvé, recherche d\'un pattern alternatif...');
  
  // Pattern alternatif pour trouver l'endroit où ajouter les contrôles de pagination
  const altDataTablePattern = /<\/DataTable>\s*\n\s*<\/Box>/;
  
  if (altDataTablePattern.test(content)) {
    content = content.replace(altDataTablePattern, `</DataTable>
    ...
    console.log('Contrôles de pagination ajoutés avec succès (pattern alternatif)');
  } else {
    console.log('Aucun pattern trouvé, ajout manuel nécessaire');
    
    // Écrire un fichier de remplacement complet
    const replacementPath = path.join(__dirname, 'Home.replacement.js');
    fs.writeFileSync(replacementPath, `import React, { useState, useEffect, useCallback } from 'react';
...
export default Home;`);
    console.log(`Fichier de remplacement complet créé: ${replacementPath}`);
    console.log('Veuillez copier manuellement ce fichier pour remplacer Home.js');
  }
}
